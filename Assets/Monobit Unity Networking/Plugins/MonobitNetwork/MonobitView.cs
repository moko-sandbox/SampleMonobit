...
namespace MonobitEngine
{
	...
	{
        protected static bool IsChanged( Component component ){
            Type type = component.GetType();
            if ( typeof( Transform ) == type ){
                return ((Transform)component).hasChanged;
            }else if ( typeof( MonobitTransformView ) == type ){
                return IsChanged( ((MonobitTransformView)component).gameObject.transform );
            }
            return true;
        }
        
        public override bool IsChangedSerializeView(){
            if ( null != observed ){
                if ( IsChanged( observed ) ) return true;
            }
            foreach ( Component component in InternalObservedComponents ){
                if ( null == component ) continue;
                
                if ( IsChanged( component ) ) return true;
            }
		...
		private void Serialize(Component component, MonobitStream stream, MonobitMessageInfo info)
        {
            ...
            new Switch(component)
                ...
                .Case<MonobitTransformView>(p =>
                {
                    ...
                    ((MonobitTransformView)p).gameObject.transform.hasChanged = false;
                })
		...
		private void ExecuteComponentOnSerialize<T>(Component component, MonobitStream stream, MonobitMessageInfo info)
        {
            ...
            if (methodInfo != null)
            {
                ...
                if (stream.isWriting)
                {
                    stream.Enqueue("MUNCMP");
                    if (componentName == "MonobitAnimatorView")
                    {
                        stream.Enqueue((Byte)mun.STREAM.STREAM_ATTRIBUTE.STREAM_ATTR_MONOBIT_ANIMATOR_VIEW_START);
                    }
                    else if(componentName == "MonobitTransformView")
                    {
                        stream.Enqueue((Byte)mun.STREAM.STREAM_ATTRIBUTE.STREAM_ATTR_MONOBIT_TRANSFORM_VIEW_START);
                    }
                    else
                    {
                        stream.Enqueue((Byte)mun.STREAM.STREAM_ATTRIBUTE.STREAM_ATTR_OTHER_OBJECT_START);
                    }
                }
                else
                {
                ...
                if (stream.isWriting)
                {
                    ...
                    if (componentName == "MonobitAnimatorView")
                    {
                        stream.Enqueue((Byte)mun.STREAM.STREAM_ATTRIBUTE.STREAM_ATTR_MONOBIT_ANIMATOR_VIEW_END);
                    }
                    else if (componentName == "MonobitTransformView")
                    {
                        stream.Enqueue((Byte)mun.STREAM.STREAM_ATTRIBUTE.STREAM_ATTR_MONOBIT_TRANSFORM_VIEW_END);
                    }
                    else
                    {
                        stream.Enqueue((Byte)mun.STREAM.STREAM_ATTRIBUTE.STREAM_ATTR_OTHER_OBJECT_END);
                    }
                }
                else
                {
