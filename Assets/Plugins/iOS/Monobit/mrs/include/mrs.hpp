...
MRS_API uint32 MRS_API_CC mrs_server_get_connection_num( MrsServer server );
     ...
     * @param type 接続の種類。MRS_CONNECTION_TYPE_TCP または MRS_CONNECTION_TYPE_UDP のどちらかを設定します
...
MRS_API void* MRS_API_CC mrs_server_get_data( MrsServer server );
     ...
     * @return 新しく割り当てられた接続
     * mrs_connect関数は厳密には接続を開始するだけで、実際のトランスポート層パケットはこの関数を呼び出しても送信されず、
     * 後のmrs_update関数の呼び出しをするときに送信されます。
     */    
